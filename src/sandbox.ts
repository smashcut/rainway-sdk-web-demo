...
class StreamWidget {
  ...
  constructor(
    private runtime: RainwayRuntime,
    ...
    this.getElement("disconnect-from-host-button").addEventListener(
      "click",
      (e) => this.disconnectFromHost()
    );
  ...
  public setUIState(newState: SandboxWidgetState) {
    ...
    this.enableInputs(
      "when-host-or-connecting",
      newState >= S.ConnectingToHost
    );
    this.enableInputs("when-host", newState >= S.ConnectedToHostNoStream);
    this.enableInputs(
      "when-ready-to-stream",
      newState === S.ConnectedToHostReadyToStream
    );
...
export class StreamSandbox {
  ...
  constructor(private numWidgets: number) {
    ...
    this.querySelector(".connect-to-relay-button").addEventListener(
      "click",
      () => this.connectToRelay()
    );
    this.querySelector(".disconnect-from-relay-button").addEventListener(
      "click",
  ...
  private setUIState(newState: SandboxState) {
    ...
    const descriptions = {
      ...
      [S.ConnectedToRelay]:
        "Connected to relay as " + this.runtime?.getPeerId(),
    };
    this.querySelector(".rainway-outer-state")!.innerText =
